...
class Local_based_attention_variation(nn.Module):
    ...
    def local_attention_scalarSub(self, q, k, v):
        q_repeated = q.repeat(1, 1, 1, k.shape[-2], 1)  # q_repeated.shape == (B, H, N, K, D)
        diff = q_repeated - k  # diff.shape == (B, H, N, K, D)
        energy = q @ diff  # energy.shape == (B, H, N, 1, K)
        scale_factor = math.sqrt(q.shape[-1])
    ...
    def local_attention_scalarAdd(self, q, k, v):
        q_repeated = q.repeat(1, 1, 1, k.shape[-2], 1)  # q_repeated.shape == (B, H, N, 1, D)
        energy = q_repeated + k  # energy.shape == (B, H, N, K, D)
        scale_factor = math.sqrt(q.shape[-1])
        attention = torch.tanh(energy / scale_factor)  # attention.shape == (B, H, N, K, D)
    ...
    def local_attention_vectorAdd(self, q, k, v):
        q_repeated = q.repeat(1, 1, 1, None, 1)  # q_repeated.shape == (B, H, N, 1, D)
        energy = q_repeated + k  # energy.shape == (B, H, N, K, D)
        scale_factor = math.sqrt(q.shape[-1])
...
class Global_based_attention_variation(nn.Module):
    ...
    def global_attention_Sub(self, q, k, v):
        ...
        sum_diff_squared = qq + inner + kk.transpose(3, 2)  # pairwise_distance.shape == (B, H, N, N)
        energy = torch.sqrt(sum_diff_squared)
        scale_factor = math.sqrt(q.shape[-1])
        attention = self.softmax(energy / scale_factor)
        # attention.shape == (B, H, N, N)
        x = (attention @ v).permute(0, 2, 1, 3)
